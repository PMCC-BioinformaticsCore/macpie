...
vignette: >
  %\VignetteIndexEntry{cross_platform_compatibility}
...
```{r violin_plots_batch24, fig.width=8, fig.height=6}
...
```{r SCTransform_batch24, cache=FALSE}
...
```{r qc_stats_batch24, fig.width=28, fig.height=6}
qc_stats <- compute_qc_metrics(mac_filtered, group_by = "combined_id", order_by = "median")
```
...
<!-- ```{r} -->
<!-- mac_filtered$combined_id <- make.names(mac_filtered$combined_id) -->
<!-- treatment_samples <- "BA.51.NO76.10" -->
<!-- control_samples <- "CB.43.EP73.0" -->
<!-- mac_filtered_nobatch <- compute_single_de(mac_filtered, treatment_samples, control_samples, method =  "limma_voom") -->
<!-- plot_volcano(mac_filtered_nobatch, max.overlaps = 10) -->
<!-- ``` -->
