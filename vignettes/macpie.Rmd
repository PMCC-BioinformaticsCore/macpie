...
**Key points**: 
  ...
  - use **plot_metadata_heatmap** to check your metadata
...
**Key points**: 
  ...
  -  use **plot_plate_layout** to check read counts
  ...
  -  use **plot_rle** to check any row/column effects
...
**Key points**: 
  ...
  -  use **compute_single_de** to perform a differential expression analysis for one treatment group vs control group
  -  use **compute_multi_de** to perform  differential expression analyses for all treatment groups vs control group
  -  volcano plot, box plot and heatmap to show results from the analyses and visualise gene expression levels  
...
```
If you prefer to see at the expression level for each replicate, you can do it by specifying logCPM - "lcpm". As we're checking log CPM, we can also check our control DMSO_0. 
