...
As described previously, MAC-seq is more variable than RNA-seq, which can influence data variability within a sample. One way to estimate this is with poisson distance of reads within a sample

```{r, fig.width = 8, fig.height = 3}
plot_distance(mac, "combined_id", treatment = "DMSO_0")
```
##### 2.2.5 Estimate of batch effect
...
```{r qc_stats, fig.width = 8, fig.height = 6}
...
By indicating one of the metrics in sd_value, mad_value, group_median, z_score, and IQR, we can use lollipop chart to show the variability in terms of z score:

```{r z_score_lollipop, fig.width = 8, fig.height = 10}
...
For example, Staurosporine_10 in three wells B02, I21 and N12, I21 is showed with the biggest distance, and it's showed in the raw reads I21 is the one with highest RNA reads. Staurosporine as a positive control is expected to have low RNA reads. So, by combing both raw read count metrics and poisson distances, we're more confident to say Staurosporine_10_I21 could be flagged as a potential outlier. 

```{r distance_plot, fig.width = 8, fig.height = 6}
...
```{r de_single_summary}
single_de_summary <- summarise_de(top_table_edgeR)
single_de_summary
```
...
Here, we can visualise treatment groups with shared differentially expressed genes. The definition of shared diffrentially expressed genes are the top 5 DE genes from each single drug comparison (treatment vs control) that are found in at least 2 different treatment groups.


This heatmap shows shared differentially expressed genes with corresponding log2FC values. We can see that three doses of compound Luminespib have been grouped together on the left, and Trametinib also grouped together on the right.
