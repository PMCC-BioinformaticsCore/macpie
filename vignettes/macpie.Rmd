...
# Load metadata
metadata <- read_metadata(file_path = project_metadata)
colnames(metadata)
...
qc_stats <- compute_qc_metrics(mac, group_by = "combined_id")
...
plot_qc_metrics(qc_stats, group_by = "combined_id", metrics = "z_score")
plot_qc_metrics(qc_stats, group_by = "combined_id", metrics = "mad_value")
plot_qc_metrics(qc_stats, group_by = "combined_id", metrics = "IQR")
...
plot_distance(mac, group_by = "combined_id", treatment = "Staurosporine_10")
plot_distance(mac, group_by = "combined_id", treatment = "DMSO_0")
plot_distance(mac, group_by ="combined_id", treatment = "SN01005979_10")
...
To show log CPM or log2FC or test metric values of the shared DE genes among different treatment groups

Top 10 DE genes from each treatment comparison that are found in at least 2 different treatment groups

```{r plot_multi_de, fig.width=10, fig.height=5}
plot_multi_de(mac, group_by = "combined_id", value = "lcpm", p_value_cutoff = 0.01, direction="up", n_genes = 10, control = "DMSO_0", by="fc")

plot_multi_de(mac, group_by = "combined_id", value = "log2FC", p_value_cutoff = 0.01, direction="up", n_genes = 10, control = "DMSO_0", by="fc")
```
