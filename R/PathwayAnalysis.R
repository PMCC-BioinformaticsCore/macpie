...
#' @param dbs name of database in encirhR such as main enrichR or
#' fish/fly/yeastenrichR
...
#' This function returns a data frame with all available
#' data sets from enrichR
#' from enrichR script, the listEnrichrSites function extracts
#' the big table of all available datasets
...
#' @param num_terms number of pathway terms to be plotted
#' @importFrom stats reorder
#' @import ggplot2
#' @export
#' @return bar plot showing top 10 enriched pathways
plot_enrich <- function(df, num_terms = 10) {
  ...
  p <- ggplot(plot_df[1:num_terms, ],
              aes(x = Hits, y = reorder(Term, -AdjP.value),
                  fill = AdjP.value)) + geom_bar(stat = "identity") + theme_bw() +
