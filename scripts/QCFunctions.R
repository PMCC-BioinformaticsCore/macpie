...
library(ggrepel)
####################### Seurat's clustering ######################
clustering <- function(mac, npcs=50) {
  if (ncol(mac)>50){
    sct <- mac %>%
...
plot_clusters <- function(mac) {
  ...
  p9 <- DimPlot(mac, group.by = "Treatment_1")
  plot(CombinePlots(plots = list(p1, p2, p3, p4, p5, p6, p7, p8, p9), nrow = 3))
}
...
plot_QC<-function(data,feature_type,treatment){
  data<-data %>%
    dplyr::select(Well_ID,Row,Column,{{feature_type}},{{treatment}}) %>% 
...
plot_grid_platelayout<-function(data, feature_type, legend_label, title_label,n_wells=384, log_scale=FALSE){
  data<-data%>%dplyr::select(Well_ID, {{feature_type}})
  if (log_scale==TRUE){
    ...
    p<-raw_map(data=data[{{feature_type}}][,1], well=data$Well_ID, plate=n_wells)+ggtitle(paste0(params$plate_code, ": ", title_label)) +
